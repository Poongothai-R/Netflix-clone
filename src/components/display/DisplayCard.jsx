    ...
    for (let i in contentData) {
        for (let j in contentData[i].dataList) {
            str = str + ',' + contentData[i].dataList[j].Genres + ',' +
                contentData[i].dataList[j].OriginalLanguage + ',' +
                contentData[i].dataList[j].DubbedLanguage;
        }
    }
                        ...
                        <div className="datacard" key={"datacard-" + idx}>
                            ...
                            <div className="card-buttons" >
                                <ImPlay2 id={"playicon-" + idx} className="play-btn"
                                         onClick={() => Navigate(`/${itm.id}/${itm1.Title}`,{ state: { data:itm1 } } ) }/>
                                <ImInfo id={"infoicon-" + idx} className="play-btn"
                                        onClick={() => setModal(<DisplayModal data={itm1} mediacategory={itm.id}/>)} />
                            </div>
                        </div>
                    ))}
            </div>
